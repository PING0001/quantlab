...
| 日内数据 | 分钟数据暂缓；做日线衍生因子 |
| 标签锚（v4 终裁） | `median(open[T+s..T+e]) / close[T] − 1`——**锚 = T 日收盘价**（2026-08-21 用户裁定，推翻代理裁定的 next_open；锚与挂单公式统一，隔夜跳空由买入侧 −3% 让利吸收） |
| 综合分（v4 终裁） | **单一 score = 0.4×pred_20d + 0.6×pred_6d**，弃百分位层（rank_score/PCT 全部移除），排序与挂价共用；阈值准入机制整体移除 |
