...
**Affiliations:** Quantigic® Solutions LLC; Free University of Tbilisi, Business School & School of Physics  
...
> **Note:** Four of the 101 alphas, namely Alpha#42, #48, #53 and #54, are delay-0 alphas. They are assumed to be traded at or as close as possible to the close of the trading day for which they are computed.
...
For our alphas we take the annualized daily Sharpe ratio **S**, daily turnover **τ**, and cents-per-share **C**. Let us label our alphas by the index **i** (i = 1,…,N) where **N = 101** is the number of alphas. For each alpha, S, τ and C are defined via:
...
Where:
- **P** = average daily P&L (in dollars)
- **σ** = daily portfolio volatility
- **V** = average daily shares traded (buys plus sells) by the i-th alpha
- **I** = total dollar investment in said alpha (the actual long plus short positions, without leverage)

More precisely, the principal of I is constant; however, I fluctuates due to the daily P&L. So, both V and I are adjusted accordingly (such that I is constant) in Equation (4). The period of time over which this data is collected is **Jan 4, 2010 – Dec 31, 2013**. For the same period we also take the sample covariance matrix of the realized daily returns for our alphas. The number of observations in the time series is 1,006, and C is nonsingular. From C we read off the daily return volatility σ and the correlation matrix:

```
ρ_ij = C_ij / (σ_i · σ_j)   (where ρ_ii = 1)
```

Note that σ_i = √C_ii, and the average daily return is given by **ν = P / I**.

Table 1 and Figure 1 summarize the data for the annualized Sharpe ratio S, daily turnover τ, 1/τ (average holding period), cents-per-share C, daily return volatility σ, annualized average daily return ν, and pair-wise correlations ρ_ij.

### 3.1. Return v. Volatility & Turnover

We run two cross-sectional regressions, both with the intercept, of ln(ν) over i) ln(σ) as the sole explanatory variable, and ii) over ln(σ) and ln(τ). The results are summarized in Tables 2 and 3. Consistently with [Kakushadze and Tulchinsky, 2015], we have no statistically significant dependence on the turnover here, while the average daily return is strongly correlated with the daily return volatility σ and we have the scaling property with **β ≈ 0.76**.
...
Where:
- **D** = specific risk
- **B** = N×K factor loadings matrix corresponding to K risk factors
- **Ω** = factor covariance matrix

In our case, we are interested in modeling the correlation matrix ρ and ascertaining whether the turnover has explanatory power for pair-wise correlations. Whether the volatility and turnover are correlated is a separate issue.

So, our approach is to take one of the columns of the factor loadings matrix as ln(τ). More precisely, a priori there is no reason why we should pick ln(τ) as opposed to ln(τ/τ*), where τ* is some normalization factor. To deal with this, let us normalize such that ln(τ) has zero cross-sectional mean, and let **u** be the unit N-vector (the intercept). Then we can construct three symmetric tensor combinations **P**, **S**, and **Q**. Let us now define a composite index **Z**, which takes X = N(N−1)/2 values, i.e., we pull the off-diagonal lower-triangular elements of a general symmetric matrix into a vector Z. This way we can construct four X-vectors **ρ**, **P**, **S** and **Q**. Now we can run a linear regression of ρ over P, S and Q. Note that P is simply the intercept (the unit X-vector), so this is a regression of ρ over P, S and Q with the intercept.

The results are summarized in Table 4. It is evident that the linear and bilinear (in ln(τ)) variables S and Q have poor explanatory power for pair-wise correlations ρ, while P (the intercept) simply models the average correlation Mean(ρ). Recall that by construction P, S and Q are orthogonal to each other, and these three explanatory variables are independent of each other.

Let us emphasize that our conclusion does not necessarily mean the turnover adds no value in the factor model context, it only means that the turnover per se does not appear to help in modeling pair-wise alpha correlations. The above analysis does not address whether the turnover adds explanatory value to modeling variances, e.g., the specific risk. Thus, a linear regression of ln(σ) over ln(τ) (with the intercept) shows nonzero correlation between these variables (see Table 5), albeit not very strong. To see if the turnover adds value via, e.g., the specific risk requires using certain proprietary methods outside of the scope of this paper.
...
Alpha#78: (rank(decay_linear(correlation(vwap, sum(adv5, 26.4719), 4.58418), 7.18088))^rank(correlation(rank(vwap), rank(volume), 5.77492)))
...
| `scale(x, a)` | Rescaled x such that sum(\|x\|) = a (default a = 1) |
...
| `ts_{O}(x, d)` | Operator O applied across the time-series for the past d days; non-integer d is converted to floor(d) |
...
Summary (using the R function `summary()`) for the annualized Sharpe ratio S, daily turnover τ, 1/τ (average holding period), cents-per-share C, daily return volatility σ, annualized average daily return ν, and pair-wise correlations ρ_ij (see Section 3). The performance figures are exclusive of any trading or transaction costs, price impact, etc.

| Quantity | Minimum | 1st Quartile | Median | Mean | 3rd Quartile | Maximum |
|----------|---------|--------------|--------|------|--------------|---------|
| S | 1.238 | 1.929 | 2.224 | 2.265 | 2.498 | 4.162 |
| τ | 0.1571 | 0.3429 | 0.4752 | 0.5456 | 0.6474 | 1.604 |
| 1/τ | 0.6235 | 1.545 | 2.104 | 2.391 | 2.916 | 6.365 |
| C | 0.1324 | 0.3125 | 0.3969 | 0.4814 | 0.5073 | 2.031 |
| 10×σ | 0.9318 | 1.194 | 1.395 | 1.747 | 2.019 | 10.44 |
| 100%×ν | 3.285 | 4.4 | 5.441 | 6.015 | 6.296 | 28.72 |
| 100%×ρ_ij | -15.09 | 7.457 | 14.31 | 15.86 | 22.91 | 87.33 |

### Table 2. Regression: ln(ν) over ln(σ)

Summary for the cross-sectional regression of ln(ν) over ln(σ) with the intercept. See Subsection 3.1 for details. Also see Figure 2.

| | Estimate | Standard error | t-statistic |
|---|----------|----------------|-------------|
| Intercept | -3.509 | 0.295 | -11.88 |
| ln(σ) | 0.761 | 0.046 | 16.65 |
| **Overall** | | | |
| Mult./Adj. R-squared | 0.737 / 0.734 | | |
| F-statistic | 277.2 | | |

### Table 3. Regression: ln(ν) over ln(σ) and ln(τ)

Summary for the cross-sectional regression of ln(ν) over ln(σ) and ln(τ) with the intercept. See Subsection 3.1 for details.

| | Estimate | Standard error | t-statistic |
|---|----------|----------------|-------------|
| Intercept | -3.435 | 0.324 | -10.60 |
| ln(σ) | 0.775 | 0.052 | 14.84 |
| ln(τ) | -0.023 | 0.040 | -0.57 |
| **Overall** | | | |
| Mult./Adj. R-squared | 0.738 / 0.732 | | |
| F-statistic | 137.8 | | |

### Table 4. Regression: ρ over P, S and Q

Summary for the cross-sectional regression of ρ over P, S and Q with the intercept. See Subsection 3.2 for details. Also see Figure 3.

| | Estimate | Standard error | t-statistic |
|---|----------|----------------|-------------|
| Intercept | 0.1587 | 0.0017 | 95.18 |
| P | 0.0067 | 0.0023 | 2.907 |
| S | 0.0474 | 0.0063 | 7.537 |
| **Overall** | | | |
| Mult./Adj. R-squared | 0.0127 / 0.0123 | | |
| F-statistic | 32.55 | | |

### Table 5. Regression: ln(σ) over ln(τ)

Summary for the cross-sectional regression of ln(σ) over ln(τ) with the intercept. See Subsection 3.2 for details. Also see Figure 4.

| | Estimate | Standard error | t-statistic |
|---|----------|----------------|-------------|
| Intercept | -6.174 | 0.062 | -100.1 |
| ln(τ) | 0.368 | 0.068 | 5.412 |
| **Overall** | | | |
...
**Figure 1.** Density plots for the annualized Sharpe ratio S, daily turnover τ, cents-per-share C, daily return volatility σ, annualized average daily return ν, and pair-wise correlations ρ_ij (see Table 1 and Section 3). The “extreme” outliers in S, ν and C are due to the delay-0 alphas (see Section 2).

**Figure 2.** Horizontal axis: ln(σ); vertical axis: ln(ν). The dots represent the data points. The straight line plots the linear regression fit ln(ν) ≈ −3.509 + 0.761 ln(σ). See Table 2.

**Figure 3.** Horizontal axis: Z; vertical axis: ρ − Mean(ρ). See Table 4 and Subsection 3.2. The numeric coefficients are the regression coefficients in Table 4.

**Figure 4.** Horizontal axis: ln(τ); vertical axis: ln(σ). The dots represent the data points. The straight line plots the linear regression fit ln(σ) ≈ −6.174 + 0.368 ln(τ). See Table 5.
