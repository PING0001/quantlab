...
freq | str | N | 数据频度 ：支持分钟(min)/日(D)/周(W)/月(M)K线，其中1min表示1分钟（类推1/5/15/30/60分钟） ，如果不填默认为D。对于分钟数据有600积分用户可以试用（请求2次），正式权限可以参考<a href="https://tushare.pro/document/1?doc_id=290"><u>权限列表说明</u></a></font> ，使用方法请参考<a href="https://tushare.pro/document/1?doc_id=234">股票分钟使用方法</a>。
