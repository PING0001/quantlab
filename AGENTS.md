...
> - **现役清单（2026-08-24）：20d 广谱 32（折存活投票定稿）/ 6d 极简 5（4+nn_gap1d）/ open2d 极简 4（3+nn_gap1d）**——无候补名单制度，换人只经六关考核（批测→画像→贡献→边际贡献门→泛化缺口→折存活投票）。**2026-08-24 审计修复批后实测**：三 IC **0.0964/0.0956/0.0611**（20d 首用 32 清单即高于 34 版），gap1d 剔死列后 0.1892；主窗口 **+30.91%/1.79/−9.03%**（修复前 +30.19%/1.64/−10.26，诚实化不伤收益；v8 版官方折验证经用户裁定跳过，清洗版/44 因子版折见 Key Decisions #6）。训练排斥为"仅训练"语义（ST/退市/封板/标签远引用越界不进训练但预测照常输出，回测宇宙不再被 T+1 信息条件化）；qfq 死列 ATR/SMA/MACD_signal/LogClose 已归档删除（表 86 列）
