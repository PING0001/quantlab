# -*- coding: utf-8 -*-
"""
_leak_check.py — 双回归 bench 泄漏断言（Task 9，2026-08-23 重写；2026-08-27 多池化）。

旧版是三分类时代的一次性污染量化脚本；本版对当前双回归体系（v8：
open2d / 6d / 20d 三模型）的盘上主窗口产物做系统性断言：

  C1 训练掩码终点   重算 buffered_train_end，断言 meta.train_end 一致；
                    末个纳入训练的日期（train_end 为排他上界）其标签最远
                    引用（T+label_window 末日）严格早于测试窗首日；
  C2 输出校准尾段   重算校准块的 calib_dates（< train_end 的留出尾段），
                    断言全部 < train_end 且标签不触及测试期；
                    meta 含 calib_slope / calib_median；
  C3 训练样本排除   预测 parquet 行集与复刻过滤链（dataset.training_panel_index
                    单源共享——IO 由本文件独立构建，不信训练进程内存）逐行
                    一致（2026-08-24 语义：ST/退市/封板仅训练排除，预测行集
                    = 全量面板，故复刻链不含这些掩码）、行数与 meta.n_pred
                    一致、日期范围 ⊆ [test_start, test_end]；
  C4 标签函数方向   compute_median_open 源码只含 shift(-d)（向后），附合成
                    面板功能对拍 + "窗口外未来价改动不影响标签"不变性。

训练协议常量（TRAIN_START/TEST_*/WARMUP/CALIB_TAIL_DAYS）单源 import 自
dataset（原 run_lgb 模块常量迁入后同源）；标签构造与训练同一实现
（dataset.compute_model_label）。
...
Usage:
    python _leak_check.py                       # 默认池（env/微盘）
    python _leak_check.py --pool mainboard_all
"""
...
import dataset
from dataset import (TRAIN_START, TEST_START, TEST_END, WARMUP_DAYS,
                     CALIB_TAIL_DAYS)
...
def check_model(
    ...
    # ---- C2 输出校准尾段 ----
    print("  [C2] 输出校准尾段 ...")
    calib_dates = [d for d in axis_dates if d < expected_end][-CALIB_TAIL_DAYS:]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="主窗口泄漏断言（56 项）")
    ...
    print("=" * 72)
    print(f"双回归 bench 泄漏断言 — 主窗口盘上产物（4 模型 | 池 {spec.name}）")
    print("=" * 72)

    check_c4_label_direction()

    # ---- IO 独立构建：自开只读连接装载，不信训练进程内存 ----
    con = duckdb.connect(str(DB_PATH), read_only=True)
    codes = union_codes(since=spec.data_since, con=con, pool=spec.name)

    # factor 面板行键（train_model 的 X 就从这里来）+ 全历史交易日（位置参考系）
    fv_df = store.load_panel(con, spec, codes=codes, cols=[])
    fv_df["date"] = pd.to_datetime(fv_df["date"])
    all_dates = sorted(fv_df["date"].unique())
    fv_idx = pd.MultiIndex.from_frame(fv_df[["date", "code"]])
    del fv_df

    # kline 只需覆盖 TRAIN_START 之后的标签窗（标签只向后引用）
    kline = dataset.load_kline(con, spec, start=str(TRAIN_START.date()))
    con.close()
    ...
    for m in sorted(MODEL_CONFIGS):
        ...
        meta_path = spec.lgb_predictions_meta_path(m)
        pred_path = spec.lgb_predictions_path(m)
        ...
        label = dataset.compute_model_label(kline, m)
        x_idx = dataset.training_panel_index(fv_idx, label, TRAIN_START,
                                             spec=spec)
        x_idx = x_idx.sort_values()   # DB 原序非日期序，shift 类计算必须先排
        axis_dates = sorted(x_idx.get_level_values("date").unique())[WARMUP_DAYS:]

        idx_date = x_idx.get_level_values("date")
        in_tw = np.asarray((idx_date >= ts) & (idx_date <= te))
        expected_pred_idx = x_idx[in_tw]

        # C1e 用：复算标签远引用越界行数（dataset.label_far_cross 与训练
        # 同式，只计训练侧 date < test_start——测试窗行 far_cross 恒真但
        # 对训练无意义）
        first_test = min(d for d in all_dates if d >= ts)
        e0 = cfg["label_window"][1]
        far_cross = dataset.label_far_cross(x_idx, e0, first_test)
        date_s = pd.Series(idx_date, index=x_idx)
        far_cross_n = int((far_cross & (date_s < ts)).sum())
